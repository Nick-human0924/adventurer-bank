...
echo "<!-- Build: v4.0.10 Time: $BUILD_TIME -->" >> dist/index.html
echo "<meta http-equiv='Cache-Control' content='no-cache, no-store, must-revalidate'>" >> dist/index.html

# 3. 提交
echo "💾 提交更改..."
git add -A
git commit -m "v4.0.10: 修复徽章加载错误 + 金币显示不一致" || echo "无更改需要提交"
...
if [ "$REMOTE_VERSION" = "v4.0.10" ]; then
    echo "✅ 部署成功！版本已同步 v4.0.10"
else
