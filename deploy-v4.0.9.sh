...
echo "<!-- Build: v4.0.9 Time: $BUILD_TIME -->" >> dist/index.html
echo "<meta http-equiv='Cache-Control' content='no-cache, no-store, must-revalidate'>" >> dist/index.html

# 3. 提交
echo "💾 提交更改..."
git add -A
git commit -m "v4.0.9: 修复金币显示不一致 + 日历时区bug" || echo "无更改需要提交"
...
if [ "$REMOTE_VERSION" = "v4.0.9" ]; then
    echo "✅ 部署成功！版本已同步 v4.0.9"
else
